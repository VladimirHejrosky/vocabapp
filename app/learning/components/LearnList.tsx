...
import { ArrowLeftRight, Divide, Eye, EyeOff, Shuffle } from "lucide-react";
import { useEffect, useState } from "react";

interface LearningWords {
  id: number;
  term: string;
  translation: string;
  example: string | null;
...
interface Props {
  initialWords: LearningWords[];
  lang: Language | undefined;
}
const LearnList = ({ initialWords, lang }: Props) => {
  const [learningTerms, setLearningTerms] = useState(initialWords);
    ...
    <>
      ...
      {learningTerms.map((term, index) => {
        ...
        return (
          <div
            key={term.id}
            className="grid grid-cols-[1fr_min-content_1fr] gap-4 p-2 border-b hover:bg-muted/10 place-items-center max-w-2xl mx-auto"
>
            <div>{firstColumn}</div>
            <div className="mx-2">
              <SpeakButton text={term.term} lang={lang} />
            </div>
            <div
